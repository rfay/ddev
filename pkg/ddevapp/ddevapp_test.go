...
func TestWebserverMariaMySQLDBClient(t *testing.T) {
	...
	serverVersions := []string{"mysql:5.7", "mysql:8.0", "mysql:8.4", "mysql:9.7", "mariadb:10.11", "mariadb:10.6", "mariadb:11.4", "mariadb:11.8", "mariadb:12.3"}
...
// file is checked too: both webservers test the resolved script with `-f` and
// reject it before php-fpm, so ddev-webserver's explanation is shown instead of
// php-fpm's "No input file specified".
