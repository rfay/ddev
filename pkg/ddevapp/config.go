...
`)
	...
	//write example Traefik .traefik_dynamic_config_extensions_template.yaml.example. Contains some sample plugin configs
	data, err := bundledAssets.ReadFile("traefik_dynamic_config_extensions_template.yaml")
	if err != nil {
		// Handle error (e.g., file not found in embedded FS)
		log.Fatal(err)
	}

	err = WriteImageDockerfile(app.GetConfigPath("/traefik/.traefik-dynamic-config-extensions-template.yaml.example"), data)
	if err != nil {
		return err
	}
